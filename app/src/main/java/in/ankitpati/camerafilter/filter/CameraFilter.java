 ...
 */
package in.ankitpati.camerafilter.filter;
...
import in.ankitpati.camerafilter.RenderBuffer;
...
public abstract class CameraFilter {
    ...
    public CameraFilter(Context context) {
        ...
        if (VERTEX_BUF == null) {
            VERTEX_BUF =
                    ByteBuffer.allocateDirect(SQUARE_COORDS.length * 4)
        ...
        if (TEXTURE_COORD_BUF == null) {
            TEXTURE_COORD_BUF =
                    ByteBuffer.allocateDirect(TEXTURE_COORDS.length * 4)
                            .order(ByteOrder.nativeOrder())
                            .asFloatBuffer();
    ...
    public final void draw(int cameraTexId, int canvasWidth, int canvasHeight) {
        // TODO move?
        ...
        GLES20.glVertexAttribPointer(
                vPositionLocation, 2, GLES20.GL_FLOAT, false, 4 * 2, VERTEX_BUF);
    ...
    void setupShaderInputs(
        ...
        GLES20.glUniform3fv(
                ...
                1,
                FloatBuffer.wrap(
                        new float[] {(float) iResolution[0], (float) iResolution[1], 1.0f}));
        ...
        GLES20.glEnableVertexAttribArray(vTexCoordLocation);
        GLES20.glVertexAttribPointer(
                vTexCoordLocation, 2, GLES20.GL_FLOAT, false, 4 * 2, textureCoord);
        ...
        GLES20.glUniform3fv(
                ...
                _iChannelResolutions.length,
                FloatBuffer.wrap(_iChannelResolutions));
    }
}
